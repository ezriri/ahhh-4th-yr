...
fig = plt.figure(1)
# set up subplot grid
gridspec.GridSpec(3,3) ## set up overall size
...
for ax in range(3):
    plt.subplot2grid((3,3), (ax,0), colspan=2) ## ((3,3) = overall shape of plot // (i,0) = location of each plot (row,column))
    plt.xlim(0,30)
    plt.ylabel(y_label[ax])
    plt.text(0.04,0.85,abc_lab[ax],fontsize=12) # ,transform=axd[mx_ax[ax]].transAxes
    for i in range(len(nc_files)):
        time = nc_dic[nc_files[i]]['time'][:]*u/1000.
        plt.plot(time,wc_dic[key_names[i][ax]],color = line_colours[i], label = names[i])
    if ax == 0:
...
plt.subplot2grid((3,3), (0,3), rowspan=3)
ax_wc_lab = plt.twinx()
bottom = np.zeros(5)
...
for bar_lab, values in nc_dic_2.items():
     plt.bar(names, values, label=bar_lab, bottom=bottom, fill=False, hatch = hatch_l[i]) ## this is plottling the stacked bars interested in
     bottom += values
     i += 1

plt.set_ylabel('Water content distribution (%)')
...
plt.legend(handles=[cloud_patch,rain_patch,ice_patch], bbox_to_anchor=(0.5, 1.1), ncol =3,  loc = 'upper right',fancybox=True, shadow=True)
plt.tight_layout()
# print to a file
